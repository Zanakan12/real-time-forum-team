...
func LoginValidationHandler(w http.ResponseWriter, r *http.Request) {
	...
	identifier := strings.ToLower(r.FormValue("username_mail")) // Peut être un email ou un username
	password := r.FormValue("password")

	fmt.Println("Tentative de connexion avec :", identifier)

	// Vérifier les informations d'identification avec la base de données
	user, err := db.UserSelectLogin(identifier, password)
	if err != nil {
